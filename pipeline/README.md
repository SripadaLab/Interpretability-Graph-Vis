...
> **This folder is the source for the site published at the repository root.**
> The root is the built output of `interpretability-graph pages`; edit `pipeline/frontend/` and rebuild rather than editing the root copies.
>
> The attributed graph JSONs are not duplicated here — they ship as `graph_data/` at the repo root, and `interpretability-graph fetch` rewrites them into `graph_files/`. What *is* included:
>
> - `graph_files/interpretations/` — cached Neuronpedia explanations for all 2 648 features, so `assign-interpretations` needs no refetch.
> - `graph_files/themes/themes.json` — the 436 LLM group themes, replayable with `apply-themes`.
...
Push `pages/` as the repo root. In Settings → Pages, either **Deploy from a branch** (`main` / root) or **GitHub Actions** using the workflow in `pages/.github/workflows/pages.yml`. Do not use GitHub's default Jekyll workflow (`jekyll-build-pages`) — this site is already static and that action often 429s. Delete `jekyll-gh-pages.yml` if GitHub added it.

Preview with `python -m http.server 8041 --directory pages`. Save downloads JSON (no write API). Feature-example tiles may still hit Anthropic / Hugging Face CDNs.
...
5. **Symmetric normalized Laplacian.** $d_i=\sum_j W_{ij}$, $D=\mathrm{diag}(d)$,
   ...
   Michael Jordan, first twelve $\lambda$: $0,\,0.025,\,0.039,\,\ldots$. Largest gaps after $\lambda_1$ sit after $\lambda_2$ and $\lambda_8$. Slider $k$ runs $2\ldots 20$; each $k$ stacks that many leading modes.
...
7. **Store.** `by_k["k"]`, `eigenvalues`, `row_normalize: false`, `affinity_path`, `knn`, `m`. Default slider $k=\min(12,k_{\max},\mathrm{round}(n/5))$ (12 on these graphs; slider to 20).

### Group themes

Clustering says which features group together; it does not say what a group is *about*. Two steps add that.

**1. Semantic interpretation assignment.** A node id is `<layer>_<feature_idx>_<ctx_idx>`, and the middle field indexes that layer's GemmaScope transcoder, so `3_7251_6` is Neuronpedia `gemma-2-2b / 3-gemmascope-transcoder-16k / 7251`. Each feature's auto-interp explanation becomes its `clerp`.

```bash
interpretability-graph assign-interpretations        # → node.clerp, graph_files/interpretations/
```

Explanation preference is `np_max-act`, then `oai_token-act-pair`, then whatever exists (matching `gonogo_task/neuronpedia_api.py`). Every candidate is cached per source, so changing preference needs no refetch. All 2 648 features in the four attributed graphs resolve; the synthetic DAG demo has no real feature indices.

**2. Theme summarization.** Each group's member interpretations go to an LLM, which returns a short theme plus a one- or two-sentence summary of what unifies them. Groups repeat across $k$ (a group at $k{=}19$ is often identical at $k{=}20$), so requests are deduplicated by member set — 1 045 group instances collapse to 485 distinct sets. Groups over 80 members are sampled evenly across layers.

```bash
interpretability-graph export-theme-requests        # → graph_files/themes/requests.jsonl
interpretability-graph apply-themes graph_files/themes/themes.json
```

`apply-themes` reads `{group_id: {theme, summary}}` and expands each theme to every $(k,\ \text{group index})$ its member set occupies, writing `metadata.spectral_clusters.themes[k][i]`. Themes describe meaning, not geometry: "Basketball vocabulary and player commentary", "Arithmetic symbols and identifier numbers", "French-language output and technical chat". Large residual bins are labelled as grab-bags with their main sub-threads named.
...
| **Group chips / cards** | Isolate one block. Same labels as **Computed · Spectral**. Each card leads with its theme and summary, then a small layer / token / repeated-feature note, then members as `L3/7251 · t6` plus that feature's interpretation. `show all N` expands to every member. Click a feature to select it in the main graph. Type $k$ or drag the slider ($2\ldots 20$). |
...
| CLI | `interpretability-graph export-svd` / `export-cosine` / `export-spectral` / `assign-interpretations` / `export-theme-requests` / `apply-themes` / `pages` / `verify` |
