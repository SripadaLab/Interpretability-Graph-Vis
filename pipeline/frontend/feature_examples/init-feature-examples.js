window.initFeatureExamples = function({containerSel, showLogits=true, showExamples=true, hideStaleOutputs=false}){
  ...
  async function renderFeature(scan, featureIndex, layerIdx){
    ...
    if (feature && feature._unavailable) return feature
  ...
  async function loadFeature(scan, featureIndex){
    try {
      var feature
      if (scan.startsWith('./')) {
        feature = await util.getFile(`${scan}/${featureIndex}.json`)
      } else if (scan.startsWith('/') || await indexFileExists(scan)) {
        feature = await loadFeatureFromBinary(scan, featureIndex)
      } else {
        feature = await util.getFile(`./features/${scan}/${featureIndex}.json`)
      }
      ...
      // Anthropic / HF feature dumps are optional; graphs still render without them.
      console.warn('feature examples unavailable', scan, featureIndex, err)
      return {
        featureIndex,
        scan,
        _unavailable: true,
        act_min: 0,
        act_max: 1.4,
        colorScale: d3.scaleSequential(d3.interpolateOranges).domain([0, 1.4]).clamp(1),
      }
    }
  }
}
