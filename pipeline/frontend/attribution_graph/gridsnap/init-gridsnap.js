window.initGridsnap = function({
  ...
  var gridsnap = {gridData}

  gridData.forEach((d, i) => {
    d.next = {...d.cur}
    d.home = {...d.cur} // preferred slot when shown again after hide
    d.hidden = !!d.hidden
    d.class = d.class === undefined ? i : d.class
  })
  
  var maxX = maxX || d3.max(gridData, d => d.cur.x + d.cur.w)
  var baseGridSizeY = gridSizeY
  function calcgridSizeX(){ 
    var w = sel.node().offsetWidth
    return w > 0 ? w / maxX : 40
  }
  var gridSizeX = calcgridSizeX()
  
  // TODO: bubble events first
  function calcGridSizeY() {
    if (!isFillContainer) return baseGridSizeY
    var h = sel.node().offsetHeight
    var rows = d3.max(gridData, d => d.cur.y + d.cur.h) || 1
    // Parent often has height:auto — don't let that collapse every tile to 0.
    if (!h) return baseGridSizeY
    return h / rows
  }
  ...
  function pushGrid(active) {
    if (active && !active.hidden) active.next = snapToGrid(active.cur)

    // Pack only visible panels so closing one collapses the empty space.
    var visible = gridData.filter(d => !d.hidden)
    var sortedGridData = d3.sort(visible, d => d != active)
  ...
  function renderPositions(active){
    gridItemSel.classed('is-hidden-panel', d => !!d.hidden)

    gridItemSel.filter(d => !d.hidden).call(renderGridItem, 'next')

    if (active && !active.hidden){
      gridItemSel.filter(d => d == active).call(renderGridItem, 'cur')
      previewSel.datum(active).call(renderGridItem, 'next')
    } else if (!active){
      // Always size the container to packed content so the main page (height:auto
      // parent) doesn't collapse to an empty viewport when isFillContainer is set.
      var maxY = d3.max(gridData.filter(d => !d.hidden), d => d.next.y + d.next.h) || 0
      var contentH = maxY * gridSizeY + pad
      if (!isFillContainer) {
        gridsnapSel.st({height: Math.max(isFullScreenY ? window.innerHeight : 0, contentH) + 'px'})
      } else if (contentH > 0) {
        gridsnapSel.st({height: Math.max(sel.node().offsetHeight || 0, contentH) + 'px'})
      }
    }
  ...
  /** Hide or show a panel by class name and reflow the grid. */
  function setHidden(className, hide) {
    var item = gridData.find(d => d.class == className)
    if (!item) {
      console.warn('gridsnap.setHidden: unknown panel', className)
      return false
    }
    var nextHidden = !!hide
    if (!!item.hidden === nextHidden) {
      // Still sync DOM / pack in case state drifted.
      renderPositions()
      return false
    }
    item.hidden = nextHidden
    if (!nextHidden) {
      // Restore preferred home slot before packing.
      if (item.home) item.next = {...item.home}
      else item.next = {...item.cur}
    }
    pushGrid()
    gridData.forEach(d => { if (!d.hidden) d.cur = {...d.next} })
    renderPositions()
    if (!nextHidden) item.resizeFn?.()
    return true
  }
  ...
  // Apply any initial hidden flags after deserialize.
  if (gridData.some(d => d.hidden)) reflow()

  gridsnap.serializeGrid = serializeGrid
  gridsnap.deserializeGrid = deserializeGrid
  gridsnap.setHidden = setHidden
  gridsnap.isHidden = isHidden
  gridsnap.reflow = reflow
